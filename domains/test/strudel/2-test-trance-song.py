# 2-test-trance-song.py

async def test_trance_song(zerg_state=None):
    """Test loading and playing the trance song through the Strudel client"""
    print("Testing trance song loading and playback")

    assert zerg_state, "This test requires valid zerg_state"

    # Get config values from zerg_state
    repl_url = zerg_state.get("strudel_repl_url").get("value")
    ws_url = zerg_state.get("strudel_ws_url").get("value")
    timeout = zerg_state.get("strudel_request_timeout").get("value")
    song_files = zerg_state.get("song_files").get("value")

    # Import from the client module that will be generated
    from strudel.client.config import StrudelClientConfig
    from strudel.client.client import StrudelClient

    # Initialize the client
    config = StrudelClientConfig(
        repl_url=repl_url,
        ws_url=ws_url,
        timeout=timeout
    )
    client = StrudelClient(config=config)

    # Test 1: Verify server is healthy
    print("\n[Test 1] Verifying server health...")
    health = await client.health_check()
    assert health.get("status") == "ok", f"Server not healthy: {health}"
    print(f"  ✅ Server is healthy")

    # Test 2: Load trance song from JS file
    print("\n[Test 2] Loading trance.js...")
    trance_path = song_files.get("trance", "strudel/songs/trance.js")
    trance_code = await client.load_song(trance_path)
    assert trance_code is not None, f"Failed to load {trance_path}"
    assert isinstance(trance_code, str), "Song code should be a string"
    assert len(trance_code) > 50, "Song code seems too short"
    print(f"  ✅ Loaded trance song ({len(trance_code)} chars)")
    print(f"     Preview: {trance_code[:100]}...")

    # Test 3: Validate loaded code can be evaluated
    print("\n[Test 3] Validating trance code...")
    result = await client.evaluate(trance_code)
    assert result.get("success"), f"Trance code evaluation failed: {result.get('error')}"
    event_count = result.get("eventCount", len(result.get("events", [])))
    assert event_count > 0, "Trance pattern generated no events"
    print(f"  ✅ Code is valid, generates {event_count} events per cycle")

    # Test 4: Send code to browser
    print("\n[Test 4] Sending code to browser...")
    set_result = await client.set_code(trance_code)
    assert set_result.get("success"), f"Failed to set code: {set_result.get('error')}"
    client_count = set_result.get("clientCount", 0)
    print(f"  ✅ Code sent to {client_count} browser client(s)")

    if client_count == 0:
        print("  ⚠️  No browser connected - skipping playback test")
        print("     Open http://localhost:7777 in a browser to enable audio playback")
    else:
        # Test 5: Play the song for specified duration
        song_duration = 8  # Play for 8 seconds
        print(f"\n[Test 5] Playing trance song for {song_duration} seconds...")
        
        play_result = await client.play_song(trance_code, duration=song_duration)
        assert play_result.get("success"), f"Playback failed: {play_result.get('error')}"
        print(f"  ✅ Song played for {song_duration} seconds")

        # Test 6: Verify playback stopped
        print("\n[Test 6] Verifying playback stopped...")
        stop_result = await client.stop()
        assert stop_result.get("success"), f"Stop failed: {stop_result.get('error')}"
        print(f"  ✅ Playback stopped successfully")

    # Test 7: Verify trance-specific elements in the song code
    print("\n[Test 7] Verifying trance characteristics...")
    trance_indicators = [
        # Trance typically has these elements
        any(x in trance_code.lower() for x in ['arpeggio', 'arpegg', '<', '>']),  # Arpeggios/sequences
        any(x in trance_code.lower() for x in ['delay', 'reverb', 'room']),  # Effects
        any(x in trance_code.lower() for x in ['sine', 'saw', 'triangle', 'square']),  # Synth sounds
        any(x in trance_code.lower() for x in ['cutoff', 'lpf', 'filter']),  # Filter sweeps
    ]
    trance_score = sum(trance_indicators)
    assert trance_score >= 2, f"Song doesn't seem very trance-like (score: {trance_score}/4)"
    print(f"  ✅ Trance characteristics score: {trance_score}/4")

    print("\n" + "=" * 50)
    print("✅ All trance song tests passed!")
    print("=" * 50)

    return True